'''
Based on the JETSCAPE/STAT package
...
def ReadDesign(FileName):
    ...
    Result["Design"] = np.loadtxt(FileName, ndmin=2)
    return Result
...
def ReadCovariance(FileName):
    ...
    Result["Matrix"] = np.loadtxt(FileName, ndmin=2)
    return Result
...
def ReadPrediction(FileName):
    ...
    Result["Prediction"] = np.loadtxt(FileName, ndmin=2).T
    return Result
