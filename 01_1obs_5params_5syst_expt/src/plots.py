...
def _posterior(
    ...
    actualvalues = [None] * len(keys)
    try:
        with open("expt/truevalues.txt", "rt") as f:
            i = 0
            for line in f:
                tokens = line.split('=')
                if len(tokens) == 2:
                    actualvalues[i] = float(tokens[-1])
                    i += 1
    except:
        pass

    for samples, key, lim, ax, av in zip(data, keys, ranges, axes.diagonal(), actualvalues):
        ...
        if av is not None:
            ax.add_line(lines.Line2D(((av**3,av**3) if ((key == 'dmin3') and dmin3_as_dmin) else (av,av)), (lim[0], 0.1*lim[0]+0.9*lim[1]), color="red"))
            ax.annotate((" %.3g " % av), xy=(av, (lim[0]+lim[1])/2.), xycoords="data", color="darkred", horizontalalignment=("left" if av < (lim[0]+lim[1])/2. else "right"))

        ax.set_xlim(lim)
        ax.set_ylim(lim)

        if (key == 'dmin3') and dmin3_as_dmin:
            samples = samples**(1/3)
    ...
    for ny, nx in zip(*np.tril_indices_from(axes, k=-1)):
        ...
        if actualvalues[ny] is not None:
            ax.add_line(lines.Line2D(ranges[nx], (actualvalues[ny]**(3 if (keys[ny] == 'dmin3') and dmin3_as_dmin else 1),)*2, color="red"))
        if actualvalues[nx] is not None:
            ax.add_line(lines.Line2D((actualvalues[nx]**(3 if (keys[nx] == 'dmin3') and dmin3_as_dmin else 1),)*2, ranges[ny], color="red"))
